...
// Helper: calculate rental days (min 1 day)
...
/**
 * CLIENT: Create booking for a vehicle
 * POST /api/bookings
 * role: client
 */
export const createBooking = async (req, res) => {
  try {
    ...
    // 1. Check vehicle exists and available
    ...
    // 2. Calculate rental days and total price
    let rentalDays;
    try {
    ...
    const totalPrice = vehicle.rentRate * rentalDays;

    // 3. Create booking (status: pending)
    const booking = await Booking.create({
      userId: req.user.userId, // client
      carId: vehicle._id,
      showroomId: vehicle.userId._id, // owner id reused as showroomId
      rentalStartDate,
      rentalStartTime,
      rentalEndDate,
      rentalEndTime,
...
export const getMyBookings = async (req, res) => {
  try {
    const bookings = await Booking.find({ userId: req.user.userId })
      .populate("carId")
      .populate("showroomId", "ownerName email contactNumber")
      .sort({ createdAt: -1 });
...
export const getOwnerBookings = async (req, res) => {
  try {
    ...
    const bookings = await Booking.find({ showroomId: ownerId })
      .populate("carId")
...
export const approveBooking = async (req, res) => {
  try {
    ...
    const booking = await Booking.findOne({ _id: id }).populate("carId");
    if (!booking) {
      return res.status(404).json({ message: "Booking not found" });
    }

    if (String(booking.showroomId) !== String(ownerId)) {
    ...
    if (booking.status !== "pending") {
      return res.status(400).json({
        message: `Booking is not in pending state (current: ${booking.status})`,
      });
    }

    // Check vehicle still available
    const vehicle = await Vehicle.findById(booking.carId);
    if (!vehicle) {
      return res.status(404).json({ message: "Vehicle not found" });
    }
    ...
    if (vehicle.availability !== "Available") {
      return res.status(400).json({
        message: `Vehicle is not available anymore (status: ${vehicle.availability})`,
      });
    }

    // Approve booking & mark vehicle as Rented Out
    booking.status = "approved";
    await booking.save();

    vehicle.availability = "Rented Out";
    await vehicle.save();

    return res
      .status(200)
      .json({ message: "Booking approved", booking });
  } catch (err) {
...
/**
 * CLIENT: Initiate return of vehicle
 * PATCH /api/bookings/:id/initiate-return
 */
export const initiateReturn = async (req, res) => {
  try {
    ...
    if (String(booking.userId) !== String(clientId)) {
      return res.status(403).json({
    ...
    if (booking.status !== "approved") {
      return res.status(400).json({
        message: `Booking must be approved before initiating return (current: ${booking.status})`,
      });
    }
    ...
    return res
      .status(200)
      .json({ message: "Return initiated", booking });
  } catch (err) {
...
/**
 * OWNER: Mark returned (with optional overdue charge)
 * PATCH /api/bookings/:id/mark-returned
 * body: { overdueHours?, overdueCharge? }
 */
export const markReturned = async (req, res) => {
  try {
    const { id } = req.params;
    const ownerId = req.user.userId;
    const { overdueHours, overdueCharge } = req.body;
    ...
    if (
      ...
      return res.status(400).json({
        message: `Booking is not in a returnable state (current: ${booking.status})`,
      });
    }

    if (overdueHours !== undefined) {
      booking.overdueHours = overdueHours;
    }
    if (overdueCharge !== undefined) {
      booking.overdueCharge = overdueCharge;
    }

    // If payment still pending, you could set pending_payment.
    if (overdueCharge && overdueCharge > 0) {
      booking.status = "pending payment";
    } else {
    ...
    return res
      .status(200)
      .json({ message: "Booking marked as returned", booking });
  } catch (err) {
